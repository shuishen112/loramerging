...
def main(
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        if args.dataset in list_math_ds:
            ...
            prompts = [
                generate_prompt(instruction, args.task, input)
        ...
        if args.dataset in list_math_ds:
            ...
            outputs = [
                o.split(
                    "Now write a Response for this Instruction. Output only the correct answer.\n\n"
                )[1]
                .split("### Response:")[1]
                .strip()
                for o in outputs
    ...
    for idx, batch in enumerate(batches):
        ...
        if args.dataset in list_math_ds:
            ...
            print(
                f"\rtest:{idx + 1}/{total} | exact acc {exact_correct}  {exact_correct / current}",
                flush=True,
            )
...
def parse_args():
    ...
    parser.add_argument(
        "--task",
        choices=["math", "qa", "rc", "prompt"],
        required=True,
        help="the task you want to evaluate",
    )
    parser.add_argument("--base_model", required=True)
    parser.add_argument(
        "--lora_weights", required=True, nargs="+", type=str, default=[]
    )
    ...
    parser.add_argument(
        "--moe", action="store_true", help="Evaluate a trained MoE model", default=False
    )
