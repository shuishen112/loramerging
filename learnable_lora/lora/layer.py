...
class LoraLayer(BaseTunerLayer):
    ...
    def set_lora_learnable_weights(self):
        print("not done")
        # set equal weight of 1/num_adapters for all adapters
        for active_adapter in self.active_adapters:
            if active_adapter not in self.lora_A.keys():
                continue
            self.lora_learnable_weights[active_adapter] = nn.Parameter(torch.tensor([1 / len(self.active_adapters)]), requires_grad=True)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            ## todo set the uniform merge
            self.set_lora_learnable_weights()
            ...
            if self.lora_mix_mode == "cat": 
                ...
                for idx,active_adapter in enumerate(self.active_adapters):
                    ...
                    dropout = self.lora_dropout[active_adapter]
                    scaling = self.scaling[active_adapter]
                    lora_B_sums += lora_B.weight * wts_after_softmax[idx] * scaling
                    lora_A_sums += lora_A.weight
                x = x.to(lora_A.weight.dtype)
