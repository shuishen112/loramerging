...
import sys
from cat_train_utils import (
    load_model, extract_answer, extract_answer_letter, extract_answer_number, load_mixed_model 
)
...
list_math_ds = ['AddSub', 'MultiArith', 'SingleEq', 'gsm8k', 'AQuA', 'SVAMP', 'gsm8k-hard', 'formats10']
...
def main(
    ...
    def evaluate(
        ...
        if args.dataset in list_math_ds:
            ...
            outputs = outputs.split("### Response:")[-1].strip().split("### Instruction:")[0]
        else:
    ...
    save_file = f'{args.outfile}-{args.dataset}.json'
    ...
    if args.lora_mix_mode:
        tokenizer, model = load_mixed_model(args)
    else:
            ...
            for data, output in zip(batch, outputs):
                ...
                if args.dataset=="natural_questions":
                    ...
                    if max_sim > 0.3:
                        correct +=1
                        flag = True
                elif args.dataset in ["ARC-Challenge", "ARC-Easy", "social_i_qa", "hellaswag", "boolq", "piqa1", "winogrande"]:
                    ...
                    if predict and label[-1] == predict[-1]:
                        correct += 1
                elif args.dataset in ["squad", "textbooks", "bioasq", "qgen"]:
                    # predict = extract_answer(args, output)
                    # if label == predict:
                    #     correct += 1
                    #     flag = True
                    label = predict = flag = None  
                else:
                    predict = label = flag = None
                    # predict = extract_answer(args, output)
                    # if label == predict:
                    #     correct += 1
                    #     flag = True
...
def load_data(args) -> list:
    ...
    """
    file_path = f"data/test_datasets/{args.task}/test.json"
    if args.task == "prompt":
...
def parse_args():
    ...
    parser.add_argument('--lora_weights',  help="CAT LoRA weights", required=True, nargs='+', type=str, default=[])
    parser.add_argument('--lora_mix_mode', help="Use this when testing TIES,DARE mixing methods", default=None)
    parser.add_argument('--batch_size', type=int, default=None)
    parser.add_argument('--load_8bit', action='store_true', default=False)
    parser.add_argument('--outfile', type=str, default=None)
    parser.add_argument('--add_special_toks', action='store_true', default=False)
    parser.add_argument('--mix_weights', help="Mixing weights for the TIES,DARE", required=False, nargs='+', type=float, default=[])
